...
class TrainingCSVLogger:
    ...
    EPISODE_FIELDS = [
        ...
        # F6: per-episode metadata for academic analysis
        ...
        # F7 (scheme v2.1, 2026-05-04): per-stage reward sum breakdown.
        # Lets us tell whether a high episodic_reward came from genuine
        # success, hovering-approach farming, or excessive lift — and
        # lets the A/B/C ablation in scheme §11 plot per-component curves.
        "r_sub_success",                 # Σ_t r_success_t (≈ 1 if succeeded else 0)
        "r_sub_approach",                # Σ_t clip(d_prev−d_now, ±0.02)
        "r_sub_grasp",                   # 1 iff stable grasp event fired this episode, else 0
        "r_sub_lift",                    # Σ_t clip(z−z_pick_ref, 0, 0.05)
        "r_sub_penalty",                 # reserved for future workspace/safety penalty
        "grasp_event_fired",             # 0/1 — did the grasp event fire at all
    ]
    ...
    #   V1 sparse:        episode reward = 1.0 success / 0.0 fail → use 0.5
    #   V2.0 dense (old): success bonus = +10 → episode reward typically > 10
    #   V2.1 hybrid (scheme 2026-05-04, current): w_s=1.0 success bonus
    #     dominates; shaping shoulder contributes ≤ ~0.4 per episode → 0.5
    #     cleanly separates success from any non-success episode.
    # 0.5 works for V1 and V2.1; override via env var HILSERL_SUCCESS_THRESHOLD.
    ...
    def log_episode(self, interaction_message: dict):
        ...
        row = {
            ...
            # F7 (scheme v2.1): per-stage reward sums forwarded by actor.py
            "r_sub_success": self._fmt(interaction_message.get("R sub success")),
            "r_sub_approach": self._fmt(interaction_message.get("R sub approach")),
            "r_sub_grasp": self._fmt(interaction_message.get("R sub grasp")),
            "r_sub_lift": self._fmt(interaction_message.get("R sub lift")),
            "r_sub_penalty": self._fmt(interaction_message.get("R sub penalty")),
            "grasp_event_fired": interaction_message.get("Grasp event fired", ""),
        }
