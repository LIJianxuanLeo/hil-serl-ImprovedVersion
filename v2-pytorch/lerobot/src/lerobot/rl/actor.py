...
def act_with_policy(
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        if done or truncated:
            ...
            # F7 (scheme v2.1): forward per-stage reward breakdown to learner
            # so episode_metrics.csv / wandb can plot success/approach/grasp/lift
            # contributions independently. The wrapper attaches this dict on
            # the terminal step's info; missing keys default to 0.0 to keep the
            # logger tolerant of envs that don't emit the breakdown.
            reward_breakdown = (
                intervention_info.get("episode_reward_breakdown", {})
                if isinstance(intervention_info, dict) else {}
            )
                ...
                python_object_to_bytes(
                    {
                        ...
                        # F7: per-stage reward sums (scheme v2.1)
                        "R sub success": float(reward_breakdown.get("success", 0.0)),
                        "R sub approach": float(reward_breakdown.get("approach", 0.0)),
                        "R sub grasp": float(reward_breakdown.get("grasp", 0.0)),
                        "R sub lift": float(reward_breakdown.get("lift", 0.0)),
                        "R sub penalty": float(reward_breakdown.get("penalty", 0.0)),
                        "Grasp event fired": int(reward_breakdown.get("grasp_event_fired", 0)),
                        **stats,
                    }
                )
            )
