...
class RAMEnv(FrankaEnv):
    def __init__(self, **kwargs):
        ...
        if keyboard is None:
            raise RuntimeError(
                "RAMEnv requires pynput keyboard listener, but pynput failed to "
                f"import (headless server?): {_PYNPUT_IMPORT_ERR}. "
                "RAMEnv is for the real robot; don't construct it on a sim/headless run."
            )
