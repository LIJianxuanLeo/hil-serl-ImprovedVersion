...
except Exception as _spacemouse_err:
    _SPACEMOUSE_IMPORT_ERR = _spacemouse_err

    class SpaceMouseExpert:  # type: ignore
        """空壳：headless / 无 hidapi 时占位，构造时给出明确报错。"""

        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "SpaceMouseExpert is unavailable on this machine "
                f"({_SPACEMOUSE_IMPORT_ERR}). Don't construct it on a sim/headless run."
            )
...
try:
    ...
    keyboard = None
    _PYNPUT_IMPORT_ERR = _pynput_err
else:
...
class KeyBoardIntervention(gym.ActionWrapper):
    def __init__(self, env, action_indices=None):
        ...
        if keyboard is None:
            raise RuntimeError(
                "KeyBoardIntervention requires pynput, but it failed to import "
                f"(headless server?): {_PYNPUT_IMPORT_ERR}. "
...
class KeyBoardIntervention2(gym.ActionWrapper):
    def __init__(self, env, action_indices=None):
        ...
        if keyboard is None:
            raise RuntimeError(
                "KeyBoardIntervention2 requires pynput, but it failed to import "
                f"(headless server?): {_PYNPUT_IMPORT_ERR}. "
                "Don't wrap this on a headless machine."
            )
