...
from examples.experiments.pick_cube_sim.staged_reward_wrapper import StagedRewardWrapper
...
class TrainConfig(DefaultTrainingConfig):
    ...
    def get_environment(self, fake_env=False, save_video=False, classifier=False, render_mode=None):
        # 自动检测 headless（无显示器 / 远程服务器）：
        # 1) MUJOCO_GL=egl/osmesa 表明走离屏渲染
        # 2) 或显式 SERVER_HEADLESS=1
        # 3) 或 DISPLAY 未设置且 SSH_CONNECTION 已设
        headless = (
            os.environ.get("MUJOCO_GL", "").lower() in ("egl", "osmesa")
            or os.environ.get("SERVER_HEADLESS") == "1"
            or (not os.environ.get("DISPLAY") and os.environ.get("SSH_CONNECTION"))
        )
        # 强制覆盖外部传入的 render_mode：
        #   - headless 一律 rgb_array（offscreen，learner/server）
        #   - 非 headless 真实 env 一律 human（弹窗，actor 端要看画面 + 键盘介入）
        #   - fake_env（learner 用的占位 env）不需要弹窗，保持 rgb_array
        if headless or fake_env:
        ...
        # ─── V2: dense staged reward ────────────────────────────────────────
        # Replaces the upstream binary 0/1 reward in PandaPickCubeGymEnv.step().
        # Per-step reward in [0, 10] + additive +10 success bonus, designed so
        # Q ≈ 15-40 dominates SAC's entropy term (~6 with temperature=1.0).
        # Mirrors hilserl-surrol-improved-v2/lerobot/.../staged_reward_wrapper.py.
        # See module docstring for the reward decomposition.
        env = StagedRewardWrapper(env, lift_target=0.1,
                                  max_episode_steps=EnvConfig.MAX_EPISODE_LENGTH)

        classifier=False
        # fake_env=True
        # env = GripperCloseEnv(env)
        if not fake_env and not headless:
...
try:
    import glfw
except Exception as _glfw_err:
    glfw = None
    _GLFW_IMPORT_ERR = _glfw_err
else:
    _GLFW_IMPORT_ERR = None
...
class KeyBoardIntervention2(gym.ActionWrapper):
    def __init__(self, env, action_indices=None):
        ...
        if glfw is None:
            raise RuntimeError(
                "KeyBoardIntervention2 requires glfw, but it failed to import "
                f"(headless server?): {_GLFW_IMPORT_ERR}. "
                "Don't wrap this on a headless machine."
            )
    ...
    def glfw_on_key(self, window, key, scancode, action, mods):
        ...
        self.current_action = [
            int(self.key_states['s']) - int(self.key_states['w']), 
            int(self.key_states['d']) - int(self.key_states['a']), 
