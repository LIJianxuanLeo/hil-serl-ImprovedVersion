...
### 档位 A：原汁原味（保留 hil-serl-sim 全部默认设置）
...
### 档位 C：消费级 GPU 极限（仅可跑通，不追求上游报告的性能）
...
```
**注意**：这套接近我们 V1/V2 的设计，性能会大幅低于上游 README 报告值。
...
### 3.2 单次训练成本估算（以 hil-serl-sim 默认设置为准）

参考库报告：30K steps ≈ 1 小时收敛。但实际复现往往需要 2-4× 该时长。**按 4 小时/run 算**：
...
**结论**：如果只是想完成 pick-and-lift 这个具体任务（而不是完整复现 SERL 上游 benchmark），**我们的 V1/V2 是性价比更高的选择** —— 不需要云 GPU，不需要远程训练，本地笔记本就能跑完。

仅当需要做严格的"与上游 hil-serl-sim 对照"实验时，才值得为它投入云 GPU 成本。
