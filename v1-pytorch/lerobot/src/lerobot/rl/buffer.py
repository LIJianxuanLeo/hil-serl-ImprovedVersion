...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int) -> BatchTransition:
        ...
        high = max(1, self.size - 1) if self.optimize_memory and self.size < self.capacity else self.size

        idx = torch.randint(low=0, high=high, size=(batch_size,), device=self.storage_device)

        if self.optimize_memory:
            # Resample indices that fall on episode boundaries, because
            # states[(idx+1) % cap] would be the first state of the *next* episode
            # rather than the true next_state for that transition.
            invalid = self.episode_ends[idx]
            max_resample_iters = 10
            for _ in range(max_resample_iters):
                n_invalid = int(invalid.sum().item())
                if n_invalid == 0:
                    break
                idx[invalid] = torch.randint(low=0, high=high, size=(n_invalid,), device=self.storage_device)
                invalid = self.episode_ends[idx]
