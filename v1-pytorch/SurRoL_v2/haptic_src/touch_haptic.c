...
HDCallbackCode HDCALLBACK DevicePoseCallback_right(void *pUserData);
HDCallbackCode HDCALLBACK DevicePoseCallback_left(void *pUserData);
...
/*
 Absolute pose callbacks — return raw HD_CURRENT_POSITION / GIMBAL_ANGLES
 without computing deltas.
 */
...
void getDevicePose_right(float* pose_info, int n3)
{
    ...
    hdScheduleSynchronous(DevicePoseCallback_right, &myinfo,
        HD_DEFAULT_SCHEDULER_PRIORITY);

    pose_info[0] = (float)myinfo.position[0];
    pose_info[1] = (float)myinfo.position[1];
    pose_info[2] = (float)myinfo.position[2];
    pose_info[3] = (float)myinfo.angle[0];
    pose_info[4] = (float)myinfo.angle[1];
    pose_info[5] = (float)myinfo.angle[2];
    pose_info[6] = (float)myinfo.button;
}
...
void getDevicePose_left(float* pose_info2, int n4)
{
    ...
    hdScheduleSynchronous(DevicePoseCallback_left, &myinfo,
        HD_DEFAULT_SCHEDULER_PRIORITY);

    pose_info2[0] = (float)myinfo.position[0];
    pose_info2[1] = (float)myinfo.position[1];
    pose_info2[2] = (float)myinfo.position[2];
    pose_info2[3] = (float)myinfo.angle[0];
    pose_info2[4] = (float)myinfo.angle[1];
    pose_info2[5] = (float)myinfo.angle[2];
    pose_info2[6] = (float)myinfo.button;
}
